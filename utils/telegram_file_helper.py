...
async def _download_file(bot, file_id: str, target: Path) -> bool:
    """Download Telegram file using aiogram's built-in two-step download.
    Step 1: get_file() returns metadata including file_path on Telegram's server.
    Step 2: bot.download_file(file_path, destination=...) streams via aiogram's
    own HTTP session — token is handled internally by aiogram, never appears
    in our code or logs.
    """
    try:
